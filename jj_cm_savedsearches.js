            ...
            */
                ...
                try {
                    ...
                    log.debug("PRODUCTION_EFFICIENCY_DATA_FETCH", {
                        ...
                        firstRecordSample: rawResults.length > 0 ? {
                            ...
                            issuedPiecesDiamond: rawResults[0].received_pieces_info,
                            lossPiecesDiamond: rawResults[0].loss_pieces_info
                        } : 'NO DATA'
                    });
                        ...
                        if (deptIds.length > 0) {
                            ...
                            startingQtyResults.forEach(record => {
                                ...
                                const qtyData = {
                                    ...
                                    received_pieces_info: parseFloat(record.received_pieces_info || 0),
                                ...
                                if (!categoryQtyMap[deptCatKey]) {
                                    categoryQtyMap[deptCatKey] = { starting_qty_gold: 0, starting_qty_diamond: 0, issued_qty_gold: 0, issued_qty_diamond: 0, loss_qty_gold: 0, loss_qty_diamond: 0, scrap_qty_gold: 0, scrap_qty_diamond: 0, balance_qty_gold: 0, balance_qty_diamond: 0, received_pieces_info: 0, loss_pieces_info: 0 };
                                }
                                Object.keys(qtyData).forEach(k => { categoryQtyMap[deptCatKey][k] += qtyData[k]; });

                                if (bagName) {
                                    if (!categoryBagQtyMap[deptCatBagKey]) {
                                        categoryBagQtyMap[deptCatBagKey] = { starting_qty_gold: 0, starting_qty_diamond: 0, issued_qty_gold: 0, issued_qty_diamond: 0, loss_qty_gold: 0, loss_qty_diamond: 0, scrap_qty_gold: 0, scrap_qty_diamond: 0, balance_qty_gold: 0, balance_qty_diamond: 0, received_pieces_info: 0, loss_pieces_info: 0, metal_purity_percent: 0 };
                                    }
                                ...
                                if (!employeeCategoryQtyMap[empCatKey]) {
                                    employeeCategoryQtyMap[empCatKey] = { starting_qty_gold: 0, starting_qty_diamond: 0, issued_qty_gold: 0, issued_qty_diamond: 0, loss_qty_gold: 0, loss_qty_diamond: 0, scrap_qty_gold: 0, scrap_qty_diamond: 0, balance_qty_gold: 0, balance_qty_diamond: 0, received_pieces_info: 0, loss_pieces_info: 0 };
                                }
            ...
            getRepairEfficiencyData(location, startDate, endDate, isRepairOnly) {
                try {
                    ...
                    // FILTER: custrecord_jj_repair_order = 'T' (repair orders only, excludes production)
                    // DATA FETCHED:
                    //   - Total Records: ${rawResults.length}
                    //   - Source Tables: CUSTOMRECORD_JJ_OPERATIONS, CUSTOMRECORD_JJ_DIRECT_ISSUE_RETURN, CUSTOMRECORD_JJ_BAG_GENERATION, CUSTOMRECORD_JJ_MANUFACTURING_DEPT, employee
                    //   - Diamond Pieces Fields: custrecord_jj_dir_issued_pieces_info, custrecord_jj_dir_loss_pieces_info (from DIRECT_ISSUE_RETURN)
                    //   - Key Filters Applied:
                    //     * custrecord_jj_repair_order = 'T' (repair orders only, excludes production)
                    //     * custrecord_jj_issued_quantity > 0 OR custrecord_jj_dir_starting_qty > 0
                    //     * Date Range: ${sqlStartDate} to ${sqlEndDate}
                    //     * Location: ${location || 'All'}
                    //   - Sample Data: ${rawResults.length > 0 ? `Dept: ${rawResults[0].department_name}, Bag: ${rawResults[0].bag_name}, Employee: ${rawResults[0].firstname} ${rawResults[0].lastname}` : 'NO DATA RETURNED'}
                        ...
                        if (deptIds.length > 0) {
                            ...
                            let startingQtyQuery = `
                                SELECT 
                                    ...
                                    BUILTIN_RESULT.TYPE_FLOAT(SUM(CASE WHEN item.class IN (${GOLD_AND_JEWELRY_CLASS_IDS.join(', ')}) THEN NVL(dir.custrecord_jj_additional_quantity, 0) ELSE 0 END)) AS balance_qty_gold,
                                    BUILTIN_RESULT.TYPE_FLOAT(SUM(CASE WHEN item.class = ${DIAMOND_ID} THEN NVL(dir.custrecord_jj_additional_quantity, 0) ELSE 0 END)) AS balance_qty_diamond,
                                    BUILTIN_RESULT.TYPE_FLOAT(SUM(CASE WHEN item.class = ${DIAMOND_ID} THEN NVL(dir.custrecord_jj_dir_issued_pieces_info, 0) ELSE 0 END)) AS received_pieces_info,
                            ...
                            summaryLog += `Category Bag Map Entries: ${Object.keys(categoryBagQtyMap).length}\n`;
